...
public class BookmarkService {
    ...
    public List<BookmarkResponse> findAll() {
        return bookmarkRepository.findAllWithQuestion()
                .stream().map(BookmarkResponse::from).collect(Collectors.toList());
    }

    @Transactional
    public BookmarkResponse create(BookmarkRequest request) {
        if (bookmarkRepository.findByQuestionId(request.getQuestionId()).isPresent()) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Question already bookmarked");
        }
    ...
    @Transactional
    public BookmarkResponse updateNote(UUID id, BookmarkNoteRequest request) {
